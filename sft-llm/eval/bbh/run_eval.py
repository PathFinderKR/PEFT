...
def compute_exact_match(predictions, targets):
    ...
    matches = [int(_normalize(p) == _normalize(t)) for p, t in zip(predictions, targets)]
    ...
    for task_name in tqdm.tqdm(all_tasks.keys(), desc="Evaluating"):
        ...
        for example, output in zip(task_examples, outputs):
            ...
            if extracted_answer:
                ...
                example["prediction"] = output.split('\n\n')[0].strip()
        ...
        performance[task_name] = compute_exact_match(predictions, targets)
