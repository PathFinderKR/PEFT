...
class hessian():
    ...
    def __init__(self, model, criterion, data=None, dataloader=None, cuda=True, batch_parser=None):
        """
        ...
        if not self.full_dataset:
            self.inputs, self.targets = self._parse_batch(self.data)

            # if we only compute the Hessian information for a single batch data, we can re-use the gradients.
            outputs = self._model_forward(self.inputs)
    ...
    def _move_to_device(self, obj):
        ...
        if isinstance(obj, (list, tuple)):
            converted = [self._move_to_device(v) for v in obj]
            return tuple(converted) if isinstance(obj, tuple) else converted
        return obj

    def _default_batch_parser(self, batch):
        if not isinstance(batch, (list, tuple)) or len(batch) != 2:
            raise ValueError(
                "Default batch parser expects a (inputs, targets) tuple/list. "
                "Please pass `batch_parser` for custom batch formats."
            )
        return self._move_to_device(batch[0]), self._move_to_device(batch[1])

    def _parse_batch(self, batch):
        if self.batch_parser is None:
            return self._default_batch_parser(batch)
        return self.batch_parser(batch, self.device)
    ...
    def dataloader_hv_product(self, v):
        ...
        for batch in self.data:
            ...
            if isinstance(inputs, dict):
                tmp_num_data = next(iter(inputs.values())).size(0)
            else:
